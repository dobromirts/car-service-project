...
import com.tsvetkov.autoservice.domain.models.view.AllUsersViewModel;
...
public class UserController {
    ...
    public ModelAndView allUsers(ModelAndView modelAndView) {
        List<AllUsersViewModel> allUsersViewModel = this.userService.findAllUsers()
                .stream().map(u -> {
                    AllUsersViewModel allUsersViewModel1 = this.modelMapper.map(u, AllUsersViewModel.class);
                    allUsersViewModel1.setAuthorities(u.getAuthorities().stream().map(RoleServiceModel::getAuthority).collect(Collectors.toSet()));
                    return allUsersViewModel1;
                }).collect(Collectors.toList());

        modelAndView.addObject("model", allUsersViewModel);
